...
const serializeActivities = (responseData: TResponseData) => {
  ...
  const serializedGithubData = github.map((activityGithub: any) =>
    aagh(activityGithub, github_files)
  );
  const serializedTwitterData = twitter.data.map((activityTwitter: any) =>
    aatw(activityTwitter)
  );
...
export const loader = async () => {
  ...
  const gh = getProvider("github") as any;
  const githubHeaders = {
  ...
  responseData.github_files = await Promise.all(
    await filesApiUrls.map((filesUrl: string) =>
      fetchConvert(filesUrl, { headers: githubHeaders })
    )
  );

  // Twitter data fetching
  const tw = getProvider("twitter") as any;
